...
for theta = theta_list
    ...
    plot(0:(size(temp(method_list_theta(1)),2)-1),temp(method_list_theta(1)),theta_linetype(theta),'DisplayName',strrep("upper limit for theta = "+theta,'_',' '));
    temp = X_1_ll_list_theta(theta);
    plot(0:(size(temp(method_list_theta(1)),2)-1),temp(method_list_theta(1)),theta_linetype(theta),'DisplayName',strrep("lower limit for theta = "+theta,'_',' '));
end
hold off
...
plt.Title = 'Z_1 limits'; % plot title
plt.BoxDim = [10, 6];
saveas(gcf,'RMPC_theta_example_Z_1.png')
%% Plot Z_2 limits
figure;
%frame_h = get(handle(gcf),'JavaFrame');
%set(frame_h,'Maximized',1);
...
plt.XLabel = 'k'; % xlabel
plt.YLabel = 'limits'; %ylabel
plt.Title = 'Z_2 limits'; % plot title
plt.BoxDim = [10, 6];
saveas(gcf,'RMPC_theta_example_Z_2.png')
%% Plot U limits
...
plt.XLabel = 'k'; % xlabel
plt.YLabel = 'limits'; %ylabel
plt.Title = 'U limits'; % plot title
plt.BoxDim = [10, 6];
saveas(gcf,'RMPC_theta_example_U.png')